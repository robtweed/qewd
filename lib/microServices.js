...
var import_qewd_up_modules = require('../up/importModules');
...
function addClient(url, application, startParams) {
  ...
  if (!this.u_services.clients[url]) {
    ...
    this.u_services.clients[url].on('ewd-registered', function() {
      ...
      // if we're using QEWD-Up, send the getRoutes message if it's to be imported
      //  application tells us the microservice name that has just been registered

      if (q.userDefined.config.qewd_up) {
        import_qewd_up_modules.call(q, application, this.token);
      }
  ...
  services_config.routes.forEach(function(service) {
    ...
    if (path) {
      ...
      var route = {
        ...
        bypassJWTCheck: service.bypassJWTCheck,
