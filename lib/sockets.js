  ...
  io.on('connection', function (socket) {
    ...
    var handleMessage = function(data) {
      ...
      if (data.jwt && data.token) {
        ...
        try {
          var payload = jwt.decode(data.token, null, true);
        }
        catch(err) {
          thisSocket.emit('ewdjs', {
            type: data.type,
            message: {
              error: 'Invalid JWT',
              disconnect: true
        ...
        if (q.u_services && q.u_services.byApplication[application] && q.u_services.byApplication[application][data.type]) {
          ...
          data.token = jwt.encode(payload, q.jwt.secret);

          console.log('socketClient sending: ' + JSON.stringify(data)); 
          socketClient.client.send(data, function(responseObj) {
            // reset the application back to the one used in the original incoming JWT
