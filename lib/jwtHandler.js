...
function masterRequest(data, socket, handleResponse) {
  ...
  if (this.u_services && this.u_services.byApplication[application] && this.u_services.byApplication[application][data.type]) {
    ...
    /*
       ...
       move this stuff to child process

          if (payload.qewd) {
            var dec = decrypt(payload.qewd, q.jwt.secret);
            try {
          ...
          delete data.token;
          // worker will have access to decrytped session
          //  session.jwt will ensure that encrypted JWT is returned by appHandler to master process
    */
  }
}
