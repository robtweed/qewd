...
function encrypt(text, secret) {
  if (!key) {
    var results = compute(algorithm, secret);
    console.log('**** jwtHandler encrypt: key = ' + results[0]);
    console.log('**** jwtHandler encrypt: iv = ' + results[1]);
    key = Buffer.from(results[0], 'hex');
    iv = Buffer.from(results[1], 'hex');
  }
    ...
    decodeJWTInWorker.call(this, data.token, function(responseObj) {
      if (responseObj.message.error) {
        socket.emit('ewdjs', {
          type: data.type,
          message: {
            error: responseObj.message.error,
            disconnect: true
          }
        });
        return;
...
function createRestSession(args, timeout) {
  var now = Math.floor(Date.now()/1000);
  var timeout = timeout || this.userDefined.config.initialSessionTimeout;
...
function validate(messageObj, noVerify) {
  //console.log('** validate using JWT: ' + messageObj.token);

  if (noVerify === true) {
    return {
      session: jwt.decode(messageObj.token, 'dummy', true)
    }
  }
  ...
  if (typeof dec === 'string' && dec.startsWith('Error: ')) {
    console.log("\n*** Unable to decrypt the JWT's secret payload, possibly due to ");
    console.log('an invalid or outdated cookie in the browser, eg before a change');
    console.log('of JWT secret.  Delete the browser cookie');
    console.log(dec);
    console.log('-----');
    dec = {};
  }
