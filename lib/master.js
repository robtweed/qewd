...
function start(params, routes) {
  ...
  if (params.use_worker_threads) {
    // use Worker Thread version
    qoper8 = require('qoper8-wt');
  }
  else {
    // use Child Process version
    qoper8 = require('ewd-qoper8');
  }

  var maxq = params.max_queue_length || 20000;
  q = new qoper8.masterProcess(maxq);

  console.log('Double ended queue max length set to ' + maxq);
  ...
  var config = {
    ...
    service_name: params.service_name || false,
    use_worker_threads: params.use_worker_threads || false,
    onWorkerStartedPath: params.onWorkerStartedPath || false
  };
