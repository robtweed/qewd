...
function start(params, routes) {
  ...
  if (config.jwt) q.jwt = config.jwt;
  ...
  if (params.u_services) {
    ...
    var qewdSocketClient = require('qewd-socketClient');
    params.u_services.forEach(function(service) {
      ...
      for (type in service.types) {
        ...
        url = serviceType.url;
        if (!q.u_services.clients[url]) {
          q.u_services.clients[url] = new qewdSocketClient();
          startParams[url] = {
            url: url,
          ...
          q.u_services.clients[url].on('ewd-registered', function() {
        ...
        q.u_services.byApplication[application][type] = {
          ...
          client: q.u_services.clients[url]
        };
        console.log('Application ' + application + ': messages of type "' + type + '" will be handled by ' + url);
      }
    });
    // now start the socket connections to the remote micro-socket servers
    for (url in q.u_services.clients) {
      q.u_services.clients[url].start(startParams[url]);
    }
  }
