...
module.exports = function() {
  ...
  this.on('message', function(messageObj, sendFn, finishedFn) {
    ...
    if (type === 'ewd-register') {
      ...
      if (messageObj.jwt) {
        if (this.jwt) {
          ...
          if (this.jwt && this.jwt.secret) params.jwt.secret = this.jwt.secret;
        }
        else {
    ...
    if (messageObj.jwt) {
      ...
      var secret;
      if (this.jwt && this.jwt.secret) secret = this.jwt.secret;
      result = this.sessions.authenticateByJWT(messageObj.token, 'noCheck', secret);
    ...
    if (type === 'ewd-reregister') {
      ...
      result = {ok: true};
      if (messageObj.jwt && secret) {
        
        delete session.jwt;
        result.token = updateJWT(session, secret);
      }
      finished(result);
      return;
    }
    ...
    if (this.handlers[application][type]) {
      ...
      var fin = function(results) {
        ...
        results = results || {};
        results.ewd_application = application;
        if (session.jwt && !results.error) {
          delete session.jwt;
          results.token = updateJWT(session, secret);
        }
        finished(results);
      };
      if (this.beforeHandlers[application]) {
