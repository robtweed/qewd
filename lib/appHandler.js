...
function loadModule(application, finished) {
  try {
    ...
    if (appModule.servicesAllowed) {
      this.servicesAllowed[application] = appModule.servicesAllowed;
    }
    // provide an initialisation point to load stuff like documentStore event handlers for indexing
    if (appModule.init && typeof appModule.init === 'function') {
      appModule.init.call(this, application);
    }
...
module.exports = function() {
  ...
  this.on('message', function(messageObj, sendFn, finishedFn) {
    ...
    //console.log('** appHandler - messageObj: ' + JSON.stringify(messageObj, null, 2));
    ...
    var result;
    if (messageObj.jwt) {
      if (messageObj.path && messageObj.path.startsWith('/qewd/importRoutes/')) {
        // ignore secret to allow transfer of secret from orchestrator
        result = handleJWT.validate.call(this, messageObj, true);
      }
      else {
    ...
    if (result.error) {
      ...
      if (messageObj.ms_requestId) {
        errorResponse.ms_requestId = messageObj.ms_requestId;
        delete errorResponse.disconnect;
      }
      finished(errorResponse);
      return;
    }
    ...
    // use application from JWT unless this is a MicroService mapped request

    application = session.application;

    if (messageObj.qewd_destination && messageObj.qewd_application) {
      //if (this.permit_application_switch && this.permit_application_switch[messageObj.qewd_application]) {
      if (process.env.mode === 'microservice') {
        application = messageObj.qewd_application;
      }
    }
    ...
    if (!this.handlers[application]) {
      ok = loadModule.call(this, application);
      if (ok.error) {
    ...
    if (service) {
      ...
      if (session && session.allowedServices && typeof session.allowedServices[service] !== 'undefined') {
      ...
      if (allowService) {
        ...
        if (this.handlers[service][type]) {
          ...
          fin = function(responseObj) {
            responseObj.ewd_application = service;
            finished(responseObj);
          };
          this.handlers[service][type].call(this, messageObj, session, send, fin);
    ...
    //console.log('application = ' + application + '; type = ' + type);
    ...
    if (this.handlers[application][type]) {
      ...
      error = {error: 'Handler not defined for ' + application + ' messages of type ' + type};
      if (this.errorMessages && this.errorMessages[application] && this.errorMessages[application].noTypeHandler) {
        errorObj = customError(this.errorMessages[application].noTypeHandler);
        if (errorObj) error = errorObj;
      }
