...
module.exports = function() {
  ...
  this.on('message', function(messageObj, sendFn, finishedFn) {
    ...
    if (messageObj.type === 'restRequest' && messageObj.method.startsWith('ws:')) {
      // map microservice QEWD WebSocket requests to target application/type
      messageObj.type = 'ewd-qoper8-express';
      messageObj.expressType = messageObj.args.type;
      messageObj.application = messageObj.method.split('ws:')[1];
      //console.log('** messageObj change to: ' + JSON.stringify(messageObj, null, 2));
    }
    ...
    if (!messageObj.method || !messageObj.method.startsWith('ws:')) {
